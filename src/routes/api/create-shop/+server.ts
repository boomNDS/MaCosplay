...
export async function POST({ locals, request }) {
	...
	try {
		if (record.MaxShop <= 0) {
			const responseToken = new Response(
				JSON.stringify({ error: 'สร้างได้สูงสุด 1 รายการเท่านั้น' }),
				{
					status: 501,
					headers: {
		...
		if (existingSlugs.length > 0) {
			return new Response(
				JSON.stringify({ error: 'Slug is already in use. Please choose another one.' }),
				{
					status: 409,
					headers: {
						'content-type': 'application/json'
		...
		const resultList = await adminClient
